...
class Decision_Tree:
    def __init__(self, data, label):
        ...
        # print(f"Entropy of label '{label}' is {entropy_label:.4f}")
        for feature in data.columns[:-1]:
            ...
            # print(f"Entropy of '{feature}' is {entropy_feature:.4f}")

            info_gain = self.information_gain(data, feature, label)
                # print(f"Information Gain of '{feature}' is {info_gain:.4f}")
                # print("-" * 50)
    ...
    def information_gain(self, data, feature, label):
        ...
        for value in data[feature].unique():
            subset = data[data[feature] == value]
            proportion = len(subset) / len(data)
            feature_entropy += proportion * self.entropy(subset[label])
